...
th.autograd.set_detect_anomaly(True)#查错用的
...
class SuperQ:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.mixer is not None:
            ...
            # chosen_action_qvals_clone = chosen_action_qvals.clone.detach()#修改，加了克隆版防止版本对不上
            # chosen_action_qvals_clone.requires_grad = True
            chosen_action_qvals = chosen_action_q_tot_vals.clone().detach()#self.mixer(chosen_action_qvals_clone, batch["state"][:, :-1])
            chosen_action_qvals.requires_grad = True
