...
from ER.PER.prioritized_memory import sample_weight
import torch.nn as nn


#1.将正态分布门更新为绝对值计算的标准差和均值，在绝对值超过门时分享他的原值
...
class Esip:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # 使用绝对值来计算门和过门
        q_td_error_abs = th.abs(masked_q_td_error).to(self.device)  #新增绝对值版本的tderror
        up_value = th.zeros((masked_q_td_error.shape[0], self.n_agents))#.to(self.device)
        self.q_mean, self.q_std,self.q_up_value = self.sum_and_sig(q_td_error_abs, up_value)#改用绝对值计算
        self.q_mean =self.q_mean.to(self.device)
        self.q_std = self.q_std.to(self.device)
        self.q_up_value = self.q_up_value.to(self.device)
        #更新，将计算结果从episode*agent扩展为q_tderror的格式episode*length*agent
        ...
        #更新,计算共享tderror的清单，耗时0.001-0.002
        share_gate_mask = q_td_error_abs >= q_up_value_clone #计算掩膜，当tderror大于gate值的时候，掩膜为1，其余为0
        ...
        #更新，计算接受共享tderror的清单，耗时0.002到0.004
        receive_list = th.zeros(share_list.shape[0], masked_q_td_error.shape[1], self.n_agents).to(self.device)#第三维存所有接受的tderror的和
        receive_list_count = th.zeros(share_list.shape[0], masked_q_td_error.shape[1],self.n_agents).to(self.device)#第三维存接受了几个tderror
        for dim in range(self.n_agents):#逐个智能体计算门掩膜，重组share数据
            ...
            share_list_dim_abs = share_list_dim.abs() #做绝对值准备过掩膜
            #制作掩膜，条件是绝对值大于等于阈值
            receive_q_mask = share_list_dim_abs >= q_up_value_clone
            ...
            #结果加入总记录
            receive_list = receive_list + receive_list_dim
        ...
        receive_list = receive_list / (receive_list_count+self.epsilon_div)
        receive_list[receive_list_count==0] = 0#将计数为0的部分改为0
       #计算接受共享tderror的清单。原版，耗时0.4-0.5
        # jieshou_list = th.zeros(share_list.shape[0], masked_q_td_error.shape[1], self.n_agents)
        # share_list_abs = th.abs(share_list) #使用绝对值过门
        # for i in range(masked_q_td_error.shape[0]):
        #     for j in range(masked_q_td_error.shape[1]):
        #         dim = 0
        #         for agent in range(self.n_agents):
        ...
        # 计算轨迹的PER优先级权重和选取比率
        masked_q_td_error = masked_q_td_error.to("cpu")
        q2_mask = q2_mask.to("cpu")
    ...
    def sum_and_sig(self, masked_q_td_error, up_value):
        ...
        #更新，改进门计算代码。从0.006进到0.0001-0.0004
        #up_value = each_batch_mean + each_batch_std
        return each_batch_mean, each_batch_std, up_value  # torch.Size([32,3])
    ...
    def select_trajectory(self, td_error, mask, t_env):
        ...
        if self.args.selected == 'all':
            ...
            #更新，将权重计算改为函数形式
            return sample_weight(selected_num=selected_num,t_env=t_env,args=self.args,td_error=td_error,mask=mask), selected_ratio
